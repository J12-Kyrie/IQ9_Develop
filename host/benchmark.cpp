...
static KernelConfig parseKernelToml(const std::string& path) {
    ...
    while (std::getline(f, line)) {
        ...
        // Section header
        if (line[0] == '[') {
            if (startsWith(line, "[[kernels.args")) {
                if (current_kernel) {
                    current_kernel->args.emplace_back();
                    subsection = "kernels.args";
                }
            } else if (startsWith(line, "[[kernels")) {
        ...
        } else if (subsection == "kernels" && current_kernel) {
        ...
        if (cfg.correctness.enabled && cfg.correctness.mode == "cpu_baseline" && cpu_baseline) {
            ...
            for (auto& bi : buffers) {
                if (bi.is_output) {
                    ...
                    // Read first input buffer with its actual size
                    std::vector<float> cpu_in;
                    for (auto& bi2 : buffers) {
                        if (!bi2.is_output) {
                            size_t in_bytes = 0;
                            clGetMemObjectInfo(bi2.buf, CL_MEM_SIZE, sizeof(size_t), &in_bytes, NULL);
                            size_t in_floats = in_bytes / sizeof(float);
                            cpu_in.resize(in_floats);
                            clEnqueueReadBuffer(queue, bi2.buf, CL_TRUE, 0, in_bytes, cpu_in.data(), 0, NULL, NULL);
                            break;
                        }
                    }

                    std::vector<float> cpu_out;
                    cpu_baseline(cpu_in, cpu_out, (int)cpu_in.size());

                    // Compare only the output-sized portion
                    size_t compare_count = std::min(float_count, cpu_out.size());
                    float max_diff = 0;
                    for (size_t i = 0; i < compare_count; i++) {
                        float d = fabsf(gpu_out[i] - cpu_out[i]);
                        if (d > max_diff) max_diff = d;
                    }
                    correct = (max_diff < cfg.correctness.tolerance) && (cpu_out.size() == float_count);
                    if (!correct) {
                        fprintf(stderr, "Correctness FAIL: max_diff=%.6f (tolerance=%.6f) cpu_out=%zu gpu_out=%zu\n",
                                max_diff, cfg.correctness.tolerance, cpu_out.size(), float_count);
                    }
                    break;
                }
            }
