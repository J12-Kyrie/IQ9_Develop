    ...
    std::string* outCorrelationId)
{
  ...
  std::string corr = root.value("correlation_id", "");
  if (outCorrelationId) *outCorrelationId = corr;
  if (corr.empty()) {
    utils::Logger::warn(kTag, "missing correlation_id");
    return false;
  }
...
int main(int argc, char** argv) {
  ...
  subscriber.setMessageHandler([&](const std::string& /*topic*/, const std::string& payload) {
    ...
    if (!processScenePayload(payload, ac, *ruleEngine, relayMutex, sceneJsonlMutex,
                             relayFd, &relayMainReady, &vqaSources, &metas, &corrId)) {
      return;
    }
    ...
    if (ac.log_scene_timing) {
      std::string corr = done["correlation_id"].get<std::string>();
      utils::Logger::info(kTag, std::string("timing corr=") + corr +
                          " total_ms=" + fmtMs3(t0, t3) +
