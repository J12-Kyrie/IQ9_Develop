    ...
    std::vector<core::EventMeta>* outMetas)
{
  ...
  for (const auto& meta : metas) {
    ...
    // Build metadata JSON from chosen (core::Alert) and meta (core::EventMeta)
    nlohmann::json meta_json = {
        {"rule", chosen.rule_name_},
        {"source_id", static_cast<int>(pkt.slot_index)},
        {"detections", nlohmann::json::array()}
    };
    for (const auto& det : meta.detections_) {
        meta_json["detections"].push_back({
            {"class", det.class_name_},
            {"track_id", det.track_id_},
            {"confidence", det.confidence_}
        });
    }
    std::string meta_str = meta_json.dump();
    uint32_t meta_len = static_cast<uint32_t>(meta_str.size());

    // Send: [meta_len][meta_json][packet]
    if (!iq9::channel::sendAll(relayFd, &meta_len, sizeof(meta_len)) ||
        !iq9::channel::sendAll(relayFd, meta_str.data(), meta_str.size()) ||
        !iq9::channel::sendAll(relayFd, &pkt, sizeof(pkt))) {
