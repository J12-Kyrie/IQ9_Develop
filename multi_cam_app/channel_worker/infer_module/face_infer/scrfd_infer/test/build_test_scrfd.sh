#!/bin/bash
# build_test_scrfd.sh — 在 QCS9075 板上编译和运行 SCRFD 推理端到端测试
# 编译链接: QnnInferencer + ScrfdDecode + FacePreprocess + mem_management
set -euo pipefail
...
MODEL="$MULTI_CAM_DIR/data/models/face/det_2.5g_qcs9075.bin"
OUT="$SCRIPT_DIR/test_scrfd_inference"

echo "=== 前置检查 ==="
[ -f "$MODEL" ]                 || { echo "FATAL: $MODEL not found"; exit 1; }
...
echo "=== 编译 test_scrfd_inference ==="
echo "  源文件:"
echo "    $SCRIPT_DIR/test_scrfd_inference.cpp"
...
g++ -std=c++17 -O2 -Wall -Wextra \
    -I/usr/include \
    "$SCRIPT_DIR/test_scrfd_inference.cpp" \
