...
GstElement* MakeElement(const char* factory_name,
  ...
  if (element == nullptr && out_error != nullptr) {
    *out_error = "Failed to create element '" + std::string(factory_name) +
                 "' as '" + element_name + "'";
  }
  return element;
}
...
bool SetEnumPropertyByNick(GstElement* element,
  ...
  const GEnumValue* enum_value =
      g_enum_get_value_by_nick(enum_class, value_nick.c_str());
  if (enum_value == nullptr) {
    g_type_class_unref(enum_class);
    if (out_error != nullptr) {
      *out_error = "Enum nick not found for " + std::string(property_name) +
                   ": " + value_nick;
    }
    return false;
  }
