...
bool LeftItemRule::isStaticPerson(int track_id, const std::vector<float>& bbox) {
  if (bbox.size() < 4) return false;
  float cx = (bbox[0] + bbox[2]) * 0.5f;
  float cy = (bbox[1] + bbox[3]) * 0.5f;

  std::lock_guard<std::mutex> lock(state_mutex_);
  auto& ps = person_states_[track_id];
  if (ps.last_cx < 0.0f) {
    ps.last_cx = cx;
    ps.last_cy = cy;
    // First-occurrence heuristic: right-side(>0.45) + bottom-touching(>0.96) = likely chair
    if (bbox[3] > 0.96f && cx > 0.45f) {
      ps.static_frames = kStaticPersonThreshold;
      return true;
    }
    return false;
  }
  float dx = cx - ps.last_cx;
  float dy = cy - ps.last_cy;
  float dist = std::sqrt(dx * dx + dy * dy);
  ps.last_cx = cx;
  ps.last_cy = cy;

  if (dist < kMinPersonDisplacement) {
    ps.static_frames++;
    return ps.static_frames >= kStaticPersonThreshold;
  }
  ps.static_frames = 0;
  return false;
}
  ...
  for (const auto& det : event.detections_) {
    if (det.class_name_ == "person") {
      if (det.confidence_ >= kMinPersonOwnerConfidence &&
          !isStaticPerson(det.track_id_, det.bbox_)) {
